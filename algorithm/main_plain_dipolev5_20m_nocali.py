...
# 定位的主程序
def location_program(a_st,a_ed,x,y,sample_index):
    ...
    if train_flag:
        ...
        # 生成训练的数据集
        if genrate_dataset:
            train_data_in,train_data_in1,train_data_out,train_data_out1 = gen_data(a_st,a_ed)
            ...
            train_data_in = np.load('train_data_in_{}.npy'.format(sample_index)) 
            train_data_in1 = np.load('train_data_in1_{}.npy'.format(sample_index)) 
            train_data_out = np.load('train_data_out_{}.npy'.format(sample_index)) 
            train_data_out1 = np.load('train_data_out1_{}.npy'.format(sample_index)) 
        ...
        for kk in np.arange(no_pos):
            print(kk+1)
            train(train_data_in1[:,15*kk:15*(1+kk)], train_data_out1, batch_size, lr, kk+1, sample_index)
    ...
    ## ------------------- error ------------------------------
    errorori = np.zeros((len(x),len(y)))             # original STAR:bearing vector error
    errorori_multi = np.zeros((len(x),len(y)))       # average original STAR:bearing vector error

    error = np.zeros((len(x),len(y)))                # ISTAR (improved STAR):bearing vector error
    errorm = np.zeros((len(x),len(y)))               # ISTAR (improved STAR):moment error

    error_multi = np.zeros((len(x),len(y)))          # Avg-ISTAR (average improved STAR):bearing vector error
    errorm_multi = np.zeros((len(x),len(y)))         # Avg-ISTAR (average improved STAR):moment error
    ...
    for i in np.arange(len(x)):
        for j in np.arange(len(y)):
            ...
            ## ------------------ fixed sensor -----------------------
            B, pos = dipoleMathplain_v2(M,x[i],y[j],z0,d)  # 8个传感器的磁感应强度和目标的真实位置
            ...
            for ii in np.arange(8):
                B[ii,:] = B[ii,:] + Be + sigma1*np.random.randn(1,3)   # 添加高斯白噪声
            
                    
            r_est1, nr_opt, r_estori, nr_ori, G = LocationStarv2(B, d)   # 根据传感器信号进行定位
            ...
            errorori[i,j] = np.linalg.norm(pos - np.multiply(r_estori,nr_ori))
            ...
            r_rec_ori = []      # 每个位置STAR预测的位置向量
            m_rec = []          # 每个位置预测的磁矩
            G_rec = []          # 每个位置的磁梯度矩阵

            for k in np.arange(len(a)):
                for h in np.arange(len(b)):
                    B, pos_true = dipoleMathplain_v3(M,x[i],y[j],z0,a[k],b[h],c,d)  # 得到磁感应强度
                    ...
                    r_est1, nr_opt, r_estori, nr_ori, G = LocationStarv2(B,d)   # 不同定位算法得到的位置向量
            ...
            r_avg_ori = np.mean(r_rec_ori,0)   # Avg STAR算法预测的位置向量
            ...
            # 误差计算
            errorori_multi[i,j] = np.linalg.norm(pos - r_avg_ori)
            #print(errorori_multi[i,j])
            ...
            test_data_in_tmp = test_data_in_tmp.reshape(-1,np.dot(np.dot(len(a),len(b)),15))  # 1x375
            test_data_in_tmp = test_data_in_tmp.tolist()
            test_data_in1.append(test_data_in_tmp)

    ## ----------------  evaluation -----------------------------
    #---------------------- bearing vector --------------------------
    mean_err_ori = np.mean(errorori.reshape(1,np.dot(len(x),len(y))))
    std_err_ori = np.std(errorori.reshape(1,np.dot(len(x),len(y))))
    rms_err_ori = rmse(errorori.reshape(1,np.dot(len(x),len(y))))
    ...
    mean_err_multi_ori = np.mean(errorori_multi.reshape(1,np.dot(len(x),len(y))))
    std_err_multi_ori = np.std(errorori_multi.reshape(1,np.dot(len(x),len(y))))
    rms_err_multi_ori = rmse(errorori_multi.reshape(1,np.dot(len(x),len(y))))
    ...
    print('STAR(bearing vector): mean error = {:.4}, std = {:.4}, RMSE = {:.4}'.format(mean_err_ori,std_err_ori,rms_err_ori))
    print('ISTAR(bearing vector): mean error = {:.4}, std = {:.4}, RMSE = {:.4}'.format(mean_err,std_err,rms_err))
    print('Average STAR(bearing vector): mean error = {:.4}, std = {:.4}, RMSE = {:.4}'.format(mean_err_multi_ori,std_err_multi_ori,rms_err_multi_ori))
    print('Average ISTAR(bearing vector): mean error = {:.4}, std = {:.4}, RMSE = {:.4}'.format(mean_err_multi,std_err_multi,rms_err_multi))
    ...
    print('ISTAR(moment): mean error = {:.4}, std = {:.4}, RMSE = {:.4}'.format(mean_err,std_err,rms_err))
    print('Average ISTAR(moment): mean error = {:.4}, std = {:.4}, RMSE = {:.4}'.format(mean_err_multi,std_err_multi,rms_err_multi))
    ...
    ## ---------------- return value -----------------------------------
    istar = test_data_in[:,9:12]
    istar_moment = test_data_in[:,12:15]
    ...
    for k in np.arange(NN_output_test1.shape[0]):
        error_multi_net.append(np.linalg.norm(pos_true_rec[k,:] - Rest_nn1[k,:]))
    ...
    print('NN-ISTAR(bearing vector): mean error = {:.4}, std = {:.4}, RMSE = {:.4}'.format(mean_err_net,std_err_net,rms_err_net))
    print('Average NN-ISTAR(bearing vector): mean error = {:.4}, std = {:.4}, RMSE = {:.4}'.format(mean_err_multi_net,std_err_multi_net,rms_err_multi_net))
    

    return istar, nn_istar, aistar, nn_aistar
